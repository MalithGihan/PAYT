...
export const registrationUser = CatchAsyncError(   
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log(user.name, activationCode);
      ...
      try {
        ...
        res.status(201).json({
          success: true,
          message: `Please check your email: ${user.email} to activete your account`,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (newUser.activationCode !== activation_code) {
        return next(new ErrorHandler("Invalide activatiom code.", 400));
      }

      const { name, email, password,homeOwnerName,address,postalNumber} = newUser.user;
...
export const socialAuth = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { email, name, avatar,address } = req.body as ISocialAuthBody;
      const user = await userModel.findOne({ email });
      if (!user) {
        const newUser = await userModel.create({ email, name, avatar,address });
...
export const updateUserInfo = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (homeOwnerName) user.homeOwnerName = homeOwnerName;
      if (address) user.address = address;
      if (postalNumber) user.postalNumber = postalNumber;
